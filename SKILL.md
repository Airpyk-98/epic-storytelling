...
description: Master viral storytelling and scriptwriting engine modeled after the high-retention, psychology-based storytelling style of PsychToonsHQ. Delivers the complete end-to-end pipeline across 3 defined output structures: Tier 1 Original Script (written to user-preferred line count), Tier 2 Split Script (derived from original via the 4-word comma-splitting rule as pure line-by-line output), and Tier 3 Image CSV (1-to-1 visual prompt mapping from the split script), plus autonomous Kaggle T4 GPU audio generation (F5-TTS) with pitch-preserved speedup.
---
